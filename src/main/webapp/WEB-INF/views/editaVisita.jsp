...
<body>
	<div class="container">
		<c:set var="vis" value="${model.vis}"></c:set>
		<h1 class="display-4">Editar Visita</h1>
		<br>
		<form:form action="${pageContext.request.contextPath}/guardarEditVisita" method="post">
					...
					<select name="profesional.id_profesional" class="form-control form-control">
						<c:forEach items="${model.listap}" var="profesional">
							<option value="${profesional.getId_profesional()}"
								${model.vis.getProfesional().getId_profesional()==profesional.getId_profesional()
					...
					<select name="cliente.id_cliente" class="form-control form-control">
						<c:forEach items="${model.listac}" var="cliente">
							<option value="${cliente.getId_cliente()}"
								${model.vis.getCliente().getId_cliente()==cliente.getId_cliente() ? 'selected' : '' }>
			...
			<div class="row">
				<div class="col-1"><a class="btn btn-secondary"
						href="${pageContext.request.contextPath}/listarVisita">Volver</a></div>
